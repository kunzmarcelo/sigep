
<script type="text/javascript">
    google.charts.load('current', {'packages': ['corechart']});
    google.charts.setOnLoadCallback(drawChart);
    function drawChart() {
        var data = google.visualization.arrayToDataTable([
            ['Mês', 'Peças Produzidas', 'Produção Determinada'],
<?php
include_once "../sigep/modell/BancoDadosPDO.class.php";
$lote = new BancoDadosPDO();

$data_teste = $data_ini;
//$data_teste = date("Y-m-d");
$data_teste2 = explode("-", $data_teste);

$data_ini = $data_teste;

$data1 = explode("-", $data_ini);
$data_fim = $data1[0] . '-' . $data1[1];

if ($data1[1] == '01')
    $mes = 'Janeiro';
if ($data1[1] == '02')
    $mes = 'Fevereiro';
if ($data1[1] == '03')
    $mes = 'Março';
if ($data1[1] == '04')
    $mes = 'Abril';
if ($data1[1] == '05')
    $mes = 'Maio';
if ($data1[1] == '06')
    $mes = 'Junho';
if ($data1[1] == '07')
    $mes = 'Julho';
if ($data1[1] == '08')
    $mes = 'Agosto';
if ($data1[1] == '09')
    $mes = 'Setembro';
if ($data1[1] == '10')
    $mes = 'Outubro';
if ($data1[1] == '11')
    $mes = 'Novembro';
if ($data1[1] == '12')
    $mes = 'Dezembro';

$data_ini = $data_teste2[0] . '-' . $data_teste2[1] . '-' . '01';
$data_fim2 = $data_fim . '-31';
while ($data_ini <= $data_fim2) {
    $matriz = $lote->listarTodosMuitosParaMuitos("detalhe_celula_produto.data, sum(detalhe_celula_produto.pecas_determinadas) AS NPECAS, sum(detalhe_celula_produto.pecas_finalizadas) AS NFEITAS ", "detalhe_celula_produto", "detalhe_celula_produto.data='$data_ini'");
    while ($dados = $matriz->fetchObject()) {
        if (!$dados->data == '') {
            $data2 = explode("-", $dados->data);

            $DATA = $data2[2] . '/' . $data2[1];
            $NPECAS = $dados->NPECAS;
            $NFEITAS = $dados->NFEITAS;
            //$media = $SOMAVALOR / 20;
            $resultado = "['$DATA',$NFEITAS,$NPECAS],";
            echo $resultado;
        }
    }
    $data_ini++;
}
?>
        ]);
        var options = {
            title: "Gráfico de acompanhamento de produção -  <?php echo $mes . ' de ' . $data1[0]; ?>",
            curveType: 'function',
            legend: {position: 'bottom'}
        };
        var chart = new google.visualization.LineChart(document.getElementById('pecas_produzidas'));
        chart.draw(data, options);
    }
</script>

